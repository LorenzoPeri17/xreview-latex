...
\author{Lorenzo Peri\\\textcolor{blue}{\href{mailto:lp@lorenzoperi.com}{lp@lorenzoperi.com}}}
\date{12/03/2026, v2.0}
...
\textit{xreview} is a \removed{fantastic}\remspace package to help \added{\LaTeX} authors review their manuscripts to respond to the \changed{annoying}{valuable} modifications suggested by referees.
...
\begin{center}
    ...
    \end{remequation}
    \makeatletter
    \addtocounter{xreview@removedeqcounter}{-1}
    \makeatother
    without altering the normal equation numbering.
    \begin{equation}
        b+c = d
...
\begin{center}
    ...
    \begin{align}
    a+b &=c \\
    \removedeqline{a+b} \remaligntab \removed{=d} \remnewline
...
\subsubsection{\writeenv{addalign} and \writeenv{remalign} environments}
...
This problem is exacerbated when dealing with equations or when a paragraph break is desired in one version but not the other. 
To help with most situations, \textit{xreview} provides the \writecommand{remspace} command and its non--breaking sibling \writecommand{remnbsp}.
For more complex situations, \writecommand{hidefromclean} will be of great help.
...
\subsection{Customizing \writeenv{remalign} and \writeenv{remalign*}}
\label{sec:remalign-custom}

There are also a few commands that enable users to customize the look of \writeenv{remalign} and \writeenv{remalign*}:
\begin{itemize}
    \item \writecommand{remalignxleftshift} [default: \texttt{0.15}\writecommand{linewidth}];
    \item \writecommand{remalignxrightshift} [default: \texttt{0.34}\writecommand{linewidth}];
    \item \writecommand{remalignstarxshift} [default: \texttt{0.34}\writecommand{linewidth}];
    \item \writecommand{remalignyshift} [default: \texttt{2ex}];
    \item \writecommand{remalignstrikecolor} [default: \writecommand{removedcolor}];
\end{itemize}

The strike-through line in \writeenv{remalign} is drawn as a \writeenv{tikzpicture} call

\begin{lstlisting}[language=TeX]
\draw[color=\remalignstrikecolor, remalignstyle] 
([xshift=\remalignxleftshift, yshift=-\remalignyshift]
eqblock.north west) 
-- 
([xshift=-\remalignxrightshift, yshift=\remalignyshift]
eqblock.south east);
\end{lstlisting}

while the \writeenv{remalign*} is drawn as
...
\texttt{remalignstyle} gives users complete \writeenv{tikz} control over the style. They are very welcome to give exact dimensions (\texttt{\writecommand{tikzset}\{remalignstyle/.style={line width=2.5pt}}\}) or multiple options (\texttt{\writecommand{tikzset}\{remalignstyle/.style={very thick, dashed}}\}), to their heart's content!
